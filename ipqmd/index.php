...
require_once __DIR__ . '/config.php'; // 引入外部配置
...
class App
{
    ...
    // 构造函数：运行时读取全局配置赋值，合法语法
    public function __construct()
    {
        global $gaode_key, $tencent_map_key, $site_referer, $site_name;
        $this->key = $gaode_key;
        $this->txkey = $tencent_map_key;
        $this->referHeader = $site_referer;
        $this->siteName = $site_name; // 新增属性接收标语
    }
    ...
    public function setInfo()
    {
        ...
        $provinceLength = mb_strlen($this->province, '8bit');
        $cityLength = mb_strlen($this->city, '8bit');
        if ($cityLength <= 9 && $provinceLength <= 9 ) {
    ...
    public function picInit()
    {
        ...
        $num = rand(1,5);  // 生成随机数
        $dst_path = $this->path . "img/bg{$num}.png";
        //传递背景图片
    ...
    public function setWeather()
    {
        ...
        $mai_path = $this->path . 'icon/weather/mai.png';
        $mai_im = imagecreatefromstring(file_get_contents($mai_path));
        list($mai_w, $mai_h) = getimagesize($mai_path);
        ...
        } elseif (strpos($this->weather['weather'], '霾') !== false) {
            imagecopy($this->drawIm, $mai_im, $weather_icon_x, $weather_icon_y, 0, 0, $mai_w, $mai_h);
        ...
        imagettftext($this->drawIm, 14, 0, 95, 145, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", $this->weather['weather']); //今天天气添加到图片
        ...
        imagettftext($this->drawIm, 13, 0, 255, 90, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", '湿度:' .  $this->weather['humidity'] . '%RH'); //当前湿度添加到图片
        imagettftext($this->drawIm, 11, 0, 160, 115, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path .  "font/msyh.ttf", '风向:' .  $this->weather['winddirection'] . '风'); //当前风向添加到图片
        imagettftext($this->drawIm, 11, 0, 255, 115, imagecolorallocate($this->drawIm, 0, 51, 78), $this->path . "font/msyh.ttf", '风力级别:' . $this->weather['windpower'] . '级'); //风力级别添加到图片
    ...
    {
        // ---------- 新增：缓存设置 ----------
        $cache_dir = __DIR__ . '/cache/';      // 缓存文件夹路径
        $cache_time = 1800;                    // 缓存有效期 30 分钟（单位秒）
        $ip = $this->ip;
        ...
        if (file_exists($cache_file) && (time() - filemtime($cache_file) < $cache_time)) {
            ...
            if ($data && isset($data['status']) && $data['status'] == 0) {
                ...
                $province = array_column($result, 'province');
                $city = array_column($result, 'city');
                $adcode = array_column($result, 'adcode');
                $this->province = array_shift($province);
                $this->city = array_shift($city);
                $this->adcode = array_shift($adcode);
                if ($this->adcode) {
                $this->getWeather();
                }
            return $this;
        ...
        curl_setopt($curl, CURLOPT_REFERER, $this->referHeader); //腾讯api白名单
        ...
        $city = array_column($result, 'city');
        $adcode = array_column($result, 'adcode');
        $this->province = array_shift($province); //省
        $this->city = array_shift($city); //市
        $this->adcode = array_shift($adcode); //获取城市代码
        // ---------- 新增：如果 API 请求成功，写入缓存 ----------
        if ($data && isset($data['status']) && $data['status'] == 0) {
            if (!is_dir($cache_dir)) {
    ...
    public function getWeather()
    {
        ...
        $this->weather['winddirection'] = $this->weather['winddirection'] != "无风向" ? $this->weather['winddirection'] : "无";
